 ...
 * 
 */
public class THUserInfoActivity extends BaseActivity {
		...
		mHandler = new Handler() {
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1 && msg.obj != null) {
					...
					((AppException) msg.obj).makeToast(THUserInfoActivity.this);
				}
			}
		};
	...
	private void loadUserInfoThread(final boolean isRefresh) {
		...
		new Thread() {
			public void run() {
				...
				try {
					THUserInfo user = ((AppContext) getApplication())
	...
	private Uri getUploadTempFile(Uri uri) {
		...
		if (storageState.equals(Environment.MEDIA_MOUNTED)) {
			...
			UIHelper.ToastMessage(THUserInfoActivity.this, "无法保存上传的头像，请检查SD卡是否挂载");
			return null;
		}
		...
		if (StringUtils.isEmpty(thePath)) {
			thePath = ImageUtils.getAbsoluteImagePath(THUserInfoActivity.this, uri);
		}
		...
		final Handler handler = new Handler() {
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1 && msg.obj != null) {
					...
					// 提示信息
					UIHelper.ToastMessage(THUserInfoActivity.this, res.getErrorMessage());
					if (res.OK()) {
						// 显示新头像
					...
					try {
						...
						if (res != null && res.OK()) {
							...
							String filename = FileUtils.getFileName(user
									.getFace());
							ImageUtils.saveImage(THUserInfoActivity.this, filename,
									protraitBitmap);
						}
						msg.what = 1;
						msg.obj = res;
