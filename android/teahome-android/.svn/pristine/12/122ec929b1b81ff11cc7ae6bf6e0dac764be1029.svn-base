...
public class BBSDetail extends BaseActivity {
	...
	private List<Threads.ThreadComment> lvCommentData = new ArrayList<Threads.ThreadComment>();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		curId = Integer.parseInt(getIntent().getStringExtra("threads_id"));
	...
	private void initView() {
		//curId = getIntent().getIntExtra("threads_id", 0);
		...
		mBack = (ImageView) findViewById(R.id.bbs_detail_back);
		mRefresh = (ImageView) findViewById(R.id.bbs_detail_refresh);
		mLinearlayout = (LinearLayout) findViewById(R.id.bbs_detail_linearlayout);
		mProgressbar = (ProgressBar) findViewById(R.id.bbs_detail_head_progress);
		mFace = (ImageView) findViewById(R.id.bbs_detail_foot_face);
		...
		mFootViewSwitcher = (ViewSwitcher) findViewById(R.id.bbs_detail_foot_viewswitcher);
		mFootPubcomment = (Button) findViewById(R.id.bbs_detail_foot_pubcomment);
		mFootPubcomment.setOnClickListener(commentpubClickListener);
		mFootEditebox = (ImageView) findViewById(R.id.bbs_detail_footbar_editebox);
		...
		mFootEditer = (EditText) findViewById(R.id.bbs_detail_foot_editer);
		...
		lvHeader = View.inflate(this, R.layout.bbs_detail_content, null);
		userface = (ImageView) lvHeader
				.findViewById(R.id.bbs_listitem_userface);
		username = (TextView) lvHeader
				.findViewById(R.id.bbs_listitem_username);
		date = (TextView) lvHeader.findViewById(R.id.bbs_listitem_date);
		commentCount = (TextView) lvHeader
				.findViewById(R.id.bbs_listitem_commentCount);
		image = (ImageView) lvHeader.findViewById(R.id.bbs_listitem_image);
		audio = (ImageView) lvHeader.findViewById(R.id.bbs_audio_controller);
		audioTime = (TextView) lvHeader.findViewById(R.id.bbs_audio_time);
		audioLayout = (LinearLayout) lvHeader
				.findViewById(R.id.bbs_audio_layout);
		content = (WebView) lvHeader.findViewById(R.id.bbs_listitem_content);
		...
		lvCommentAdapter = new ListViewBBSCommentAdapter(this, lvCommentData,
				R.layout.bbs_detail_comment_listitem);
		mLvComment = (PullToRefreshListView) findViewById(R.id.bbs_detail_commentlist);
		...
		mLvComment
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					public void onItemClick(AdapterView<?> parent, View view,
						...
						// 跳转--回复评论界面
						UIHelper.showCommentReply(BBSDetail.this, curId,
				...
				.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
					public boolean onItemLongClick(AdapterView<?> parent,
						...
						if (uid == com.getAuthorId()) {
							final Handler handler = new Handler() {
								public void handleMessage(Message msg) {
									if (msg.what == 1) {
										...
										UIHelper.ToastMessage(BBSDetail.this,
												res.getErrorMessage());
									} else {
										((AppException) msg.obj)
												.makeToast(BBSDetail.this);
									}
								}
							};
							...
							UIHelper.showCommentOptionDialog(BBSDetail.this,
	...
	private void initData() {
		...
		mHandler = new Handler() {
			public void handleMessage(Message msg) {
				...
				if (msg.what == 1) {
					String name = bbsDetail.getNickname();
					if(name == null || name.equals("")){
						name = bbsDetail.getUsername();
					}
					username.setText(name);
					username.setOnClickListener(faceClickListener);
					date.setText(StringUtils.friendly_time(bbsDetail.getCreate_time()));
					commentCount.setText(bbsDetail.getComments().size() + "");

					String body = UIHelper.WEB_STYLE + bbsDetail.getContent();
					...
					// 加载用户头像
					String faceURL = THURLs.THREAD_USER_ICON_IMG_URL + bbsDetail.getThumb();
					...
					// 加载图片
					String imgSmall = THURLs.THREAD_INFO_IMG_URL + bbsDetail.getImages();
					...
					// 发送通知广播
					if (msg.obj != null) {
						UIHelper.sendBroadCast(BBSDetail.this,
								(Notice) msg.obj);
					}
				} else if (msg.what == 0) {
					UIHelper.ToastMessage(BBSDetail.this,
					...
					((AppException) msg.obj).makeToast(BBSDetail.this);
				}
			}
		};
		this.loadBBSThreadDetail(curId, mHandler, false);
		...
		mCommentHandler = new Handler() {
			public void handleMessage(Message msg) {
				...
				if (msg.what >= 0) {
					ArrayList<Threads.ThreadComment> list = (ArrayList<Threads.ThreadComment>) msg.obj;
					...
					case UIHelper.LISTVIEW_ACTION_SCROLL:
						lvSumData += msg.what;
						if (lvCommentData.size() > 0) {
							for (Threads.ThreadComment com1 : list) {
								boolean b = false;
								for (Threads.ThreadComment com2 : lvCommentData) {
					...
					// 发送通知广播
//					if (notice != null) {
//						UIHelper.sendBroadCast(BBSDetail.this, notice);
//					}
				} else if (msg.what == -1) {
					// 有异常--也显示更多 & 弹出错误消息
					curLvDataState = UIHelper.LISTVIEW_DATA_MORE;
		...
		new Thread() {
			public void run() {
				...
				if (action == UIHelper.LISTVIEW_ACTION_REFRESH
					...
					isRefresh = true;
				try {
					bbsDetail = ((AppContext) getApplication()).getBBSDetail(
							id, isRefresh);
					
					ArrayList<Threads.ThreadComment> comments = bbsDetail.getComments();
					msg.what = comments.size();
					msg.obj = comments;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = -1;
	 ...
	 * @param handler
	 */
	private void loadBBSThreadDetail(final int threadId, final Handler handler,
		...
		new Thread() {
			public void run() {
				...
				try {
					bbsDetail = ((AppContext) getApplication()).getBBSDetail(
							threadId, isRefresh);
					msg.what = (bbsDetail != null && bbsDetail.getId() != null) ? 1
							: 0;
					msg.obj = (bbsDetail != null) ? bbsDetail.getNotice()
							: null;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = -1;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == UIHelper.REQUEST_CODE_FOR_RESULT) {
			Threads.ThreadComment comm = (Threads.ThreadComment) data
	...
	private void initGridView() {
		...
		mGridView = (GridView) findViewById(R.id.bbs_detail_foot_faces);
	...
	private View.OnClickListener refreshClickListener = new View.OnClickListener() {
		public void onClick(View v) {
			hideEditor(v);
			loadBBSThreadDetail(curId, mHandler, true);
...
//						bbsDetail.getUsername(), bbsDetail.getUsername());
			}
		}
	};
