...
public class ListViewQuestionAnswerAdapter extends BaseAdapter {
	...
	private List<Question.Answer> 				listItems;//数据集合
	 ...
	 * @param resource
	 */
	public ListViewQuestionAnswerAdapter(Context context, List<Question.Answer> data,int resource) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//设置文字和图片
		Question.Answer answer = listItems.get(position);
		String faceURL = THURLs.THREAD_USER_ICON_IMG_URL + answer.getThumb();
		if(faceURL.endsWith("portrait.gif") || StringUtils.isEmpty(faceURL)){
			listItemView.face.setImageResource(R.drawable.widget_dface);
		}else{
			bmpManager.loadBitmap(faceURL, listItemView.face);
		}
		listItemView.face.setTag(answer);//设置隐藏参数(实体类)
		listItemView.face.setOnClickListener(faceClickListener);
		String username = answer.getNickname();
		...
		listItemView.content.setTag(answer);//设置隐藏参数(实体类)
...
//			tv.setText(context.getString(R.string.comment_reply_title, answer.getReplies().size()));
...
//		if(answer.getRefers().size() > 0){
//			//引用内容
//			for(Refer refer : answer.getRefers()){
